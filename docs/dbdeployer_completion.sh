...
__dbdeployer_handle_go_custom_completion()
{
    ...
    # Calling ${words[0]} instead of directly dbdeployer allows handling aliases
    args=("${words[@]:1}")
    # Disable ActiveHelp which is not supported for bash completion v1
    requestComp="DBDEPLOYER_ACTIVE_HELP=0 ${words[0]} __completeNoDesc ${args[*]}"
...
_dbdeployer_downloads_add-remote()
{
    ...
    flags+=("--change-user-agent")
    local_nonpersistent_flags+=("--change-user-agent")
    flags+=("--minimal")
    local_nonpersistent_flags+=("--minimal")
    flags+=("--overwrite")
    local_nonpersistent_flags+=("--overwrite")
...
_dbdeployer_downloads_add-url()
{
    last_command="dbdeployer_downloads_add-url"
...
_dbdeployer_downloads_import()
{
    ...
    flags+=("--merge-imported")
    local_nonpersistent_flags+=("--merge-imported")
    flags+=("--retries-on-failure=")
    two_word_flags+=("--retries-on-failure")
    local_nonpersistent_flags+=("--retries-on-failure")
    local_nonpersistent_flags+=("--retries-on-failure=")
